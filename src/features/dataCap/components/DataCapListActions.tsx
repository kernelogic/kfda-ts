...
import {
  dataCapDestroyManyApiCall,
  dataCapFindManyApiCall,
} from 'src/features/dataCap/dataCapApiCalls';
import { dataCapExporterMapper } from 'src/features/dataCap/dataCapExporterMapper';
import { dataCapFilterInputSchema } from 'src/features/dataCap/dataCapSchemas';
...
export default function DataCapListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof dataCapFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.dataCapCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.dataCapImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        dataCapExporterMapper(data.dataCaps, context),
        dictionary.dataCap.fields,
        'dataCaps',
      );
      toast({
        description: dictionary.dataCap.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.dataCap.destroyMany.noSelection);
      }

      return dataCapDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.dataCap.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/data-cap/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.dataCap.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.dataCap.destroyMany.confirmDescription,
            selectedCount,
          )}
