...
import {
  dataSourceDestroyManyApiCall,
  dataSourceFindManyApiCall,
} from 'src/features/dataSource/dataSourceApiCalls';
import { dataSourceExporterMapper } from 'src/features/dataSource/dataSourceExporterMapper';
import { dataSourceFilterInputSchema } from 'src/features/dataSource/dataSourceSchemas';
...
export default function DataSourceListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof dataSourceFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.dataSourceCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.dataSourceImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        dataSourceExporterMapper(data.dataSources, context),
        dictionary.dataSource.fields,
        'dataSources',
      );
      toast({
        description: dictionary.dataSource.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.dataSource.destroyMany.noSelection);
      }

      return dataSourceDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.dataSource.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/data-source/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.dataSource.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.dataSource.destroyMany.confirmDescription,
            selectedCount,
          )}
