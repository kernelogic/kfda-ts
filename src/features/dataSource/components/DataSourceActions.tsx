...
import { dataSourceDestroyManyApiCall } from 'src/features/dataSource/dataSourceApiCalls';
...
export function DataSourceActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.dataSourceUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return dataSourceDestroyManyApiCall([dataSource.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.dataSource.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/data-source/${dataSource.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/data-source/${dataSource.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.dataSource.destroy.confirmTitle}
