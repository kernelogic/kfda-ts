...
import {
  dataSetDestroyManyApiCall,
  dataSetFindManyApiCall,
} from 'src/features/dataSet/dataSetApiCalls';
import { dataSetExporterMapper } from 'src/features/dataSet/dataSetExporterMapper';
import { dataSetFilterInputSchema } from 'src/features/dataSet/dataSetSchemas';
...
export default function DataSetListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof dataSetFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.dataSetCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.dataSetImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        dataSetExporterMapper(data.dataSets, context),
        dictionary.dataSet.fields,
        'dataSets',
      );
      toast({
        description: dictionary.dataSet.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.dataSet.destroyMany.noSelection);
      }

      return dataSetDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.dataSet.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/data-set/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.dataSet.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.dataSet.destroyMany.confirmDescription,
            selectedCount,
          )}
