...
import { dataSetDestroyManyApiCall } from 'src/features/dataSet/dataSetApiCalls';
...
export function DataSetActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.dataSetUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return dataSetDestroyManyApiCall([dataSet.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.dataSet.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/data-set/${dataSet.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/data-set/${dataSet.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.dataSet.destroy.confirmTitle}
